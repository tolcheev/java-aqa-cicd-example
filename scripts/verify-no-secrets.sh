...
pattern="BEGIN (RSA |EC |OPENSSH )?PRIVATE KEY|Bearer[[:space:]]+eyJ|https://(prod[.-])?cinescope\\.t-qa\\.ru|((api[_-]?key|secret|access[_-]?token|vault[_-]?(token|dev[_-]?root[_-]?token[_-]?id))[[:space:]]*[:=][[:space:]]*['\"]?[A-Za-z0-9_./+=-]{20,})"
